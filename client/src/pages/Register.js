...
const Register = () => {
  ...
  const validateForm = () => {
    ...
    if (formData.role === 'student' && !formData.batch) newErrors.batch = 'Batch is required';
    setErrors(newErrors);
    ...
    <div className={`min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 ${darkMode ? 'dark bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">Join HostelHub and get started</p>
        </div>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <input id="name" name="name" type="text" value={formData.name} onChange={handleChange}
                className={`input mt-1 ${errors.name ? 'border-red-500' : ''}`} placeholder="Enter your name" />
            ...
            <div>
              ...
              <input id="email" name="email" type="email" value={formData.email} onChange={handleChange}
                className={`input mt-1 ${errors.email ? 'border-red-500' : ''}`} placeholder="Enter your email" />
            ...
            <div>
              ...
              <input id="phone" name="phone" type="text" value={formData.phone} onChange={handleChange}
                className="input mt-1" placeholder="Enter phone number" />
            </div>
            ...
            {formData.role === 'student' && (
              <div>
                ...
                <input id="batch" name="batch" type="text" value={formData.batch} onChange={handleChange}
                  className={`input mt-1 ${errors.batch ? 'border-red-500' : ''}`} placeholder="e.g. CSE-2024" />
                {errors.batch && <p className="mt-1 text-sm text-red-600">{errors.batch}</p>}
              </div>
            )}

            <button type="submit" disabled={loading} className="btn btn-primary w-full btn-lg">
              {loading ? <><LoadingSpinner size="sm" className="mr-2" /> Registering...</> : 'Register'}
            </button>
